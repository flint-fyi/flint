...
describe("createDiskBackedLinterHost", () => {
	...
	describe("watchFile", () => {
		...
		it("watches missing files through create-delete-create", async () => {
			...
			const onEvent = vi.fn();
			using _ = host.watchFile(filePath, onEvent, { pollingInterval: 10 });
		...
		it("ignores other files", async () => {
			...
			const onEvent = vi.fn();
			using _ = host.watchFile(targetPath, onEvent, { pollingInterval: 10 });
		...
		it("watches directory", async () => {
			...
			const onEvent = vi.fn();
			fs.mkdirSync(dirPath, { recursive: true });
			using _ = host.watchFile(dirPath, onEvent, { pollingInterval: 10 });
	...
	describe("watchDirectory", () => {
		it("watches directories non-recursively", async () => {
			...
			const onEvent = vi.fn();
			using _ = host.watchDirectory(directoryPath, onEvent, {
				recursive: false,
			});
		...
		it("ignores .git directories within watched paths", async () => {
			...
			const onEvent = vi.fn();
			using _ = host.watchDirectory(baseDir, onEvent, { recursive: true });
