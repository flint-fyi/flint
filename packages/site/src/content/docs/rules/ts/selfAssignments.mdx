...
```ts
declare let flags: Record<string, unknown>;
// ---cut---
...
```ts
declare let count: number;
declare const otherCount: number;
// ---cut---
count &&= otherCount;
```

```ts
declare const defaultFlag: string;
declare let flag: string;
// ---cut---
flag ||= defaultFlag;
```
...
```ts
declare let data: string;
declare const fallbackData: string;
// ---cut---
data ??= fallbackData;
```
