...
Reports lazy quantifiers that have no effect because the quantifier matches a fixed number of times.
When a quantifier has fixed bounds (min equals max), there is no choice in how many times to match, so the lazy modifier `?` is irrelevant.
