...
It defaults to `"separate-type-imports"`.
...
## JSX and React Caveat

React's classic JSX transform can require the `React` import at runtime even when TypeScript syntax only mentions it in a type annotation.
For `.tsx` files, this rule does not report imports from `"react"` when JSX syntax is present.

Examples of code this rule allows in `.tsx` files:
...
- [`importTypeSideEffects`](./importTypeSideEffects) reports imports such as `import { type A, type B }` that may emit empty side-effect imports under `verbatimModuleSyntax`.
