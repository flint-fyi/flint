...
When `true`, irregular whitespace inside string literals is allowed.
Defaults to `true`.

```ts
// With skipStrings: true (default), this is allowed
const value = "text with non-breaking space";
```
...
```ts
// With skipRegExps: true, this is allowed
const pattern = / /;
```
