...
- `BigInt(123)` can be written as `123n`
- `Boolean(true)` is redundant; use `true` directly
- `Number("123")` can be written as `123`
- `String(123)` can be written as `"123"` or `` `${123}` ``
...
If you prefer the explicit constructor syntax for consistency in your codebase, or if you're maintaining a codebase where constructor calls are used as a deliberate style choice to make type coercion more explicit, you may disable this rule.
