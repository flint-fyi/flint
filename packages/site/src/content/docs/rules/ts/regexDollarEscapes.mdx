...
If you intentionally rely on the behavior where `$` followed by an unrecognized character produces a literal `$`, you might disable this rule.
However, explicitly escaping as `$$` is clearer and recommended.
