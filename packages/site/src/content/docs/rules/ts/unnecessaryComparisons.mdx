...
If your project relies on dynamic getters or other side effects that could make seemingly-identical expressions return different values, you might need to disable this rule for those specific cases.

For NaN checking, use `Number.isNaN(value)` instead of `value !== value`.
