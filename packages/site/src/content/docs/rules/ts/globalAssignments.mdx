...
```ts
// @errors: 2322 2539 2739 -- TS catches read-only global reassignment
undefined = 1;
...
```ts
// @errors: 2539 -- TS catches read-only global modification
let count = 5;
undefined += count;
...
```ts
// @errors: 2322 -- 'undefined' inferred as literal type, not assignable from number
let count = 5;
