...
By default, Flint uses [Formatly](https://github.com/JoshuaKGoldberg/formatly) to detect and run your project's formatter after linting.
Formatly supports Biome, Deno, dprint, Oxfmt, and Prettier.
Flint checks formatting without writing files unless `--fix` is provided, and skips formatting when no formatter is detected.
By passing in `--skip-formatting`, you can disable that execution of the formatter.
You may wish to do this for performance and/or if you run your formatter separately from Flint.
