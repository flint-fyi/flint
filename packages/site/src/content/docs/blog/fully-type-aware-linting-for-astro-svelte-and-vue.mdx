...
That includes full typed linting without any additional configuration.
To provide this for users, we needed to finally solve long-standing questions around how to fully type extension languages and their embedded code with TypeScript.
...
It's unfortunately not solveable by straightforward techniques like concatenating all JavaScript expressions into one big file.
...
[Johnson Chu](https://github.com/johnsoncodehk)'s [`tsslint`](https://github.com/johnsoncodehk/tsslint) ("The lightest TypeScript semantic linting solution in JS") used Volar.js to solve the problem of type-aware linting for language plugins.
It achieved support for loading Volar.js language plugins for Astro, MDX, Vue, and Vue Vine.
...
However, they use drastically different approaches: svelte-check does ABC API thing, whereas ngtsc does DEF API thing.
...
It handles such as LSP integration, routing of LSP requests to the appropriate embedded language service, type-checking powered by TypeScript, and much more -- so that authors can avoid each reimplementing those areas of tooling.
