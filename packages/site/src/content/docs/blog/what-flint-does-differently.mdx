...
Flint's rules take a more modern approach.
They allow any [Standard Schema](https://standardschema.dev)-compliant validator for rule options, which enables the use of TypeScript-friendly schema validation libraries such as [Zod](https://zod.dev):
...
export const someRule = typescriptLanguage.createRule({
	options: {
		option: z.string().default("Hooray!"),
	},
	setup(context) {
		// ...
	},
...
Each option schema must accept `undefined` so that users can enable the rule without explicitly configuring its options.
